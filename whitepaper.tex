...
Sequencing times and costs are continually dropping, with on-the-ground sequencing used during the recent West African Ebola and Zika epidemics \citep{quick2016real,faria2017epidemic}.
Rapid methods by which to analyse, interpret, and disseminate results must complement this sepeed of sequencing.

Nextstrain consists of data curation, analysis and visualisation components:
Python scripts maintain a database representative of all available sequences and related metadata, sourced from public repositories such as NCBI (www.ncbi.nlm.nih.gov), GISAID (www.gisaid.org) and ViPR (www.viprbrc.org), as well as GitHub repositories and other sources of genomic data.
A suite of tools perform phylodynamic analysis \citep{voltz2013}, including subsampling, alignment, phylogenetic inference, temporal dating of ancestral nodes and discrete trait geographic reconstruction, including inference of the most likely transmission events.
...
Visualisation of these analysis is available through \href{http://nextstrain.org}{nextstrain.org}.
...
Internal node colours indicate inferred transmissions on the tree, which are also drawn as links between demes on the map.
It is important to make known the uncertainty of such reconstructions.
The confidence is conveyed by matching colour saturation to the confidence of that trait, and by displaying all relevant information when one hovers over the corresponding branch or isolate on the tree.
Despite this, sampling bias can obscure transmission links, and in cases of extreme bias we have chosen not to display the inferred states.
...
Despite no known cases of human to human transmission, the high mortality rate of $~30\%$ \citep{li2014epidemiology} makes the threat of mutations which facilitate human to human transmission of extreme concern (e.g. see \citep{devries2017three}).
Nextstrain allows monitoring of experimentally determined mutations, informing appropriate interventions and control measures.
...
Derived data, such as phylogenetic trees, metadata and screenshots are available, and private metadata added via CSV files.
